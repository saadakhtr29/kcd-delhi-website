...
const tracks = [
    {
        ...
        headerBg: "bg-[#F97316]",
        sessions: [
            { time: "09:00 - 09:30", title: "Opening Keynote", speaker: "TBA" },
            { time: "09:30 - 10:00", title: "Session 1", speaker: "TBA" },
            { time: "10:00 - 10:30", title: "Session 2", speaker: "TBA" },
            { time: "10:30 - 11:00", title: "Session 3", speaker: "TBA" },
            { time: "11:00 - 11:30", title: "Tea Break", speaker: "" },
            { time: "11:30 - 12:00", title: "Session 4", speaker: "TBA" },
            { time: "12:00 - 12:30", title: "Session 5", speaker: "TBA" },
            { time: "12:30 - 13:30", title: "Lunch Break", speaker: "" },
            { time: "13:30 - 14:00", title: "Session 6", speaker: "TBA" },
            { time: "14:00 - 14:30", title: "Session 7", speaker: "TBA" },
            { time: "14:30 - 15:00", title: "Session 8", speaker: "TBA" },
            { time: "15:00 - 15:30", title: "Closing Keynote", speaker: "TBA" },
        ],
    },
    {
        name: "Breakout",
...
function TrackTable({ track, isActive }: { track: typeof tracks[0]; isActive: boolean }) {
    return (
        <div className={`flex-shrink-0 w-full transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 absolute'}`}>
            <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div className={`${track.headerBg} px-4 md:px-6 py-3 md:py-4 flex items-center gap-2 md:gap-3`}>
                    <MapPin className="w-5 h-5 md:w-6 md:h-6 text-white" />
                    <h3 className="text-lg md:text-2xl font-bold text-white">{track.name}</h3>
                </div>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className={`px-6 py-4 text-sm border-b border-gray-100 ${
                                            isBreak ? "text-gray-600 italic font-medium" : "text-gray-900 font-semibold"
                                        }`}>
                                            {session.title}
                                        </td>
                        ...
                        return (
                            <div
                                ...
                                className={`p-3 border-b border-gray-100 ${
                                    isBreak ? "bg-gray-100" : index % 2 === 0 ? "bg-white" : "bg-gray-50"
                                }`}
                            >
                                <div className={`inline-block text-xs font-semibold ${track.bgColor} text-white px-2 py-1 rounded-full mb-2`}>
                                    {session.time}
                                </div>
                                <div className={`text-sm ${isBreak ? "text-gray-600 italic" : "text-gray-900 font-semibold"}`}>
                                    {session.title}
                                </div>
                                {session.speaker && (
